...
import static library.unitestvsexceptionhandling.example4.PredicateWithException.unchecked;
...
public class GuestListJava8 {
	...
	public static void main(String[] args) {
		...
		private Function<List<Guest>, Stream<Guest>> MAIN_FILTER_GUESTS = (list)->
			list.stream()
					.filter(guest-> unchecked((Guest g) -> {
				return checkLegal(g);
			}).test(guest));
	...
	public void printListOfExpectedFemaleGuests(List<Guest> invited) {
		MAIN_FILTER_GUESTS.apply(invited)
